...
//模态框的构造函数继承ComponentBase,就是说有个全局组件的配置和模态框的函数配置
export interface VxeModalConstructor extends VxeComponentBase, VxeModalMethods {
  //
  props: VxeModalProps //props是外部数据，传统组件都有的东西
  //setupContext是泛型编程的其中一种,第一个参数是emit的类型
  context: SetupContext<VxeModalEmits> //context是外部的东西,包括attr,emit,expose,slots
...
export interface ModalMethods {
  ...
  //模态框
  //
  //dispatchEvent的作用是统一状态管理
  dispatchEvent(type: ValueOf<VxeModalEmits>, params: any, evnt?: Event): void //dom事件
  /**
   * 手动打开窗口
   */
  open(): Promise<any>
...
//
export type VxeModalEmits = ['update:modelValue', 'show', 'hide', 'before-hide', 'close', 'confirm', 'cancel', 'zoom']
...
export interface ModalController {
  ...
  alert(
    content: VxeModalPropTypes.Content,
    title?: VxeModalPropTypes.Title,
    options?: VxeModalDefines.ModalOptions,
  ): Promise<ModalEventTypes>
  /**
   * 创建提示框
   * @param options 参数
   */
